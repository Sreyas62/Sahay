...
export function EducationScreen({ onBack, llmService, whisperService }: EducationScreenProps) {
  ...
  const loadChatHistory = async () => {
    try { const chats = await ChatHistoryService.getAllChats('education'); setChatHistory(chats); }
    catch (error) { console.error('Error loading chat history:', error); }
  };
  const saveCurrentChat = async () => {
    if (!currentChatId || messages.length <= 1) return;
    try { await ChatHistoryService.updateChatMessages('education', currentChatId, messages); await loadChatHistory(); }
    catch (error) { console.error('Error saving chat:', error); }
  };
  const createNewChat = async () => { setMessages(initialMessages); setShowQuickQuestions(true); setCurrentChatId(null); setShowHistory(false); };
  const loadChat = async (chatId: string) => {
    try {
      const chat = await ChatHistoryService.getChat('education', chatId);
      if (chat) { setMessages(chat.messages.length > 0 ? chat.messages : initialMessages); setSelectedLanguage(chat.language as SupportedLanguage); setCurrentChatId(chat.id); setShowQuickQuestions(chat.messages.length === 0); setShowHistory(false); }
    } catch (error) { console.error('Error loading chat:', error); Alert.alert('Error', 'Failed to load chat'); }
  };
  const deleteChat = async (chatId: string) => {
    try { await ChatHistoryService.deleteChat('education', chatId); await loadChatHistory(); if (currentChatId === chatId) { createNewChat(); } }
  ...
  const handleSendText = async (text: string, isVoice: boolean = false) => {
    ...
    if (!currentChatId) {
      try { const newChat = await ChatHistoryService.createChat('education', text, selectedLanguage); setCurrentChatId(newChat.id); }
