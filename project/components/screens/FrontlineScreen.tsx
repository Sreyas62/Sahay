...
const quickActions = [
  { id: 1, icon: '🏥', label: 'Health Visit', color: 'bg-emerald-100 text-emerald-600' },
  { id: 2, icon: '💊', label: 'Medicine Info', color: 'bg-blue-100 text-blue-600' },
  { id: 3, icon: '📋', label: 'Record Keeping', color: 'bg-purple-100 text-purple-600' },
  { id: 4, icon: '👶', label: 'Child Care', color: 'bg-pink-100 text-pink-600' }
];
...
const initialMessages: Message[] = [
  {
    id: 1,
    type: 'assistant',
    content: 'Namaste ASHA Worker! आपका स्वागत है। I\'m here to support you with health information, record keeping, and community health guidance. How can I assist you today?',
    timestamp: new Date()
  }
];

interface FrontlineScreenProps {
  onBack: () => void;
  llmService: LLMService;
  whisperService: WhisperService;
}

export function FrontlineScreen({ onBack, llmService, whisperService }: FrontlineScreenProps) {
  ...
  const loadChatHistory = async () => {
    try { const chats = await ChatHistoryService.getAllChats('frontline'); setChatHistory(chats); }
    catch (error) { console.error('Error loading chat history:', error); }
  };
  const saveCurrentChat = async () => {
    if (!currentChatId || messages.length <= 1) return;
    try { await ChatHistoryService.updateChatMessages('frontline', currentChatId, messages); await loadChatHistory(); }
  ...
  const loadChat = async (chatId: string) => {
    try {
      const chat = await ChatHistoryService.getChat('frontline', chatId);
  ...
  const deleteChat = async (chatId: string) => {
    try { await ChatHistoryService.deleteChat('frontline', chatId); await loadChatHistory(); if (currentChatId === chatId) { createNewChat(); } }
  ...
  const handleQuickAction = (label: string) => {
    const userMessage: Message = {
      ...
      type: 'user',
      content: `I need help with ${label.toLowerCase()}`,
      timestamp: new Date()
    };
    ...
    const assistantMessage: Message = {
      ...
      type: 'assistant',
      content: `I'll help you with ${label.toLowerCase()}. Please tell me more details.`,
      timestamp: new Date()
    };
  ...
  const handleSendText = async (text: string, isVoice: boolean = false) => {
    ...
    if (!currentChatId) {
      try { const newChat = await ChatHistoryService.createChat('frontline', text, selectedLanguage); setCurrentChatId(newChat.id); }
    ...
    } catch (error) { Alert.alert('Error', 'Failed to generate response.'); setMessages(prev => prev.map(msg => msg.id === assistantMessageId ? { ...msg, content: 'Sorry, I encountered an error.' } : msg)); }
      ...
      >
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerTitle}>
            ...
            <Text style={styles.headerText}>Frontline Workers</Text>
          </View>
        ...
        <View style={styles.languageButtons}>
          {(['en', 'hi', 'ml', 'kn'] as SupportedLanguage[]).map((lang) => (
            ...
            >
              ...
              >
                {lang === 'en' ? 'English' : lang === 'hi' ? 'हिंदी' : lang === 'ml' ? 'മലയാളം' : 'ಕನ്ನಡ'}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
      ...
      <View style={styles.inputContainer}>
        {!isVoiceMode ? (
          <>
            ...
            >
              <LinearGradient
                colors={!inputText.trim() || isGenerating || isTranscribing ? ['#D1D5DB', '#D1D5DB'] : ['#A855F7', '#EC4899']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
